  ...
  useEffect(() => {
    if (!isLoading) {
      if (!isAuthenticated) {
        ...
        // If logged in but not branch manager, redirect to their role's dashboard
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-gray-400 text-sm font-medium">Initializing Manager Session...</p>
        </div>
      </div>
    );
  }

  if (!isAuthenticated || user?.role !== 'BRANCH_MANAGER') {
    return null;
  }
      ...
      >
        <div className="h-full flex items-center justify-between px-10">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden md:flex items-center gap-2 px-3.5 py-2 bg-gray-50 border border-gray-200 rounded-xl">
              <Store size={15} className="text-gray-400" />
              <span className="text-[13px] font-bold text-gray-700">{user?.branch?.name || 'Local Branch'}</span>
            </div>
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search branch records..."
                className="bg-gray-50 border border-gray-200 rounded-xl py-2.5 pl-10 pr-5 text-[13px] text-gray-700 font-medium placeholder:text-gray-400 focus:outline-none focus:border-[#f5c800] focus:ring-2 focus:ring-[#f5c800]/20 transition-all w-64"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-3.5 py-2 bg-green-50 border border-green-100 rounded-xl">
              ...
              <span className="text-[12px] font-semibold text-green-600 tracking-tight">Active Duty</span>
            </div>
